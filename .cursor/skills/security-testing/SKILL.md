...
- Use repo search, `gh` code scanning alerts if present (`gh api repos/{owner}/{repo}/code-scanning/alerts?state=open`), and Dependabot alerts for dependency context.
- If a dedicated security-review subagent/skill exists for *diff* review, use this skill for whole-repo audits; use the subagent for PR-scoped review.
