...
- Prefer `gh` + local diff tools. For security-heavy PRs, you may also run the security-testing skill on touched surfaces only.
- Do not launch unrelated subagents unless the user asks. Deep mode is the exception: asking for it *is* the request, and the review passes are what it consists of.
- Where separate agents are unavailable, run the review and grading passes in **separate conversations**, carrying only the diff, the intent, and the rubric between them. Grading in the context that produced the fixes is materially weaker — say so in the report rather than presenting the number as independent.
