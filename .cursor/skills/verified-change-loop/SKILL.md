...
- Dispatch reviewer and patcher as separate agents. The isolation is what the loop is buying; running both in this context reproduces the shape without the value.
- Where separate agents are unavailable, run each role in its own conversation and carry only the diff, the intent, and the rubric between them. Never carry prior scores. If the roles could not be isolated, say so alongside the score rather than presenting it as independent.
