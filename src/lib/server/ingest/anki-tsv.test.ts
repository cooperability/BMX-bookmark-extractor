...
describe('the real Anki exports', () => {
	it('parses to exactly 457 records, not 4629 lines', () => {
		const anthro = parseAnkiExport(read(ANTHRO), OWNER);
		const compsci = parseAnkiExport(read(COMPSCI), OWNER);
	...
	it('reads notetype from the declared column, so no <div> continuation lines leak in', () => {
		const { notes } = parseAnkiExport(read(COMPSCI), OWNER);
	...
	it('derives a URL- and markup-safe id from GUIDs that contain < and %', () => {
		const { notes } = parseAnkiExport(read(ANTHRO), OWNER);
	...
	it('reports no warnings on the real corpus', () => {
		expect(parseAnkiExport(read(ANTHRO), OWNER).warnings).toEqual([]);
		expect(parseAnkiExport(read(COMPSCI), OWNER).warnings).toEqual([]);
	});

	it('is idempotent on ids, so re-import updates rather than duplicates', () => {
		const first = parseAnkiExport(read(COMPSCI), OWNER).notes;
		const second = parseAnkiExport(read(COMPSCI), OWNER).notes;
		expect(first.map((n) => n.id)).toEqual(second.map((n) => n.id));
	});

	it('gives two owners of the same shared deck disjoint ids', () => {
		// nodes.id is a global primary key and Anki GUIDs travel with shared decks,
		// so a GUID-only digest would collide the second importer with the first.
		const raw = read(COMPSCI);
		const mine = parseAnkiExport(raw, OWNER).notes;
		const theirs = parseAnkiExport(raw, OTHER).notes;

		expect(mine.map((n) => n.ankiGuid)).toEqual(theirs.map((n) => n.ankiGuid));

		const overlap = new Set(mine.map((n) => n.id));
		expect(theirs.some((n) => overlap.has(n.id))).toBe(false);
	});
});
...
describe('the preamble drives column mapping', () => {
	it('follows declared positions instead of assuming them', () => {
		...
		const { notes } = parseAnkiExport(raw, OWNER);
	...
	it('falls back to a content id when no guid column is declared', () => {
		...
		const edited = parseAnkiExport(raw.replace('back', 'edited back'), OWNER);
		expect(edited.notes[0].id).not.toBe(notes[0].id);
		// Owner-scoped on this path too, or identical cards would collide across tenants.
		expect(parseAnkiExport(raw, OTHER).notes[0].id).not.toBe(notes[0].id);
	});
	...
	it('uses the #deck fallback when no deck column is declared', () => {
		...
		expect(parseAnkiExport(raw, OWNER).notes[0].deck).toBe('Imported');
	});
});
...
describe('malformed input', () => {
	it('warns about short rows instead of dropping them', () => {
		...
		const { notes, warnings } = parseAnkiExport(raw, OWNER);
	...
	it('warns on an empty front field', () => {
		...
		expect(parseAnkiExport(raw, OWNER).warnings.some((w) => w.includes('empty front'))).toBe(true);
	});

	it('returns nothing for an empty file', () => {
		expect(parseAnkiExport('', OWNER)).toEqual({ notes: [], warnings: [] });
	});

	it('returns nothing for a preamble with no records', () => {
		expect(parseAnkiExport(PREAMBLE, OWNER)).toEqual({ notes: [], warnings: [] });
	});

	it('strips a UTF-8 BOM so the first preamble line still matches', () => {
		const raw = ['\uFEFF#separator:tab', '#guid column:1', 'g1\tfront\tback'].join('\n');
		expect(parseAnkiExport(raw, OWNER).notes[0].ankiGuid).toBe('g1');
	});
