...
const MESSAGES: Record<MoveCheck | 'empty' | 'none' | 'stale', [number, string]> = {
	ok: [200, ''],
	here: [200, ''],
	...
	none: [409, 'Nothing to face right now: every door you can reach is open or sealed.'],
	stale: [409, 'That encounter expired. Try the door again.']
};

/** A refused move or encounter, as a status the client can show. */
export function refusal(reason: MoveCheck | 'empty' | 'none' | 'stale') {
